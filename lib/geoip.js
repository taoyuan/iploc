...
function lookup6(ip) {
	...
	function readip(line, offset) {
		...
		for (let i = 0; i < 2; i++) {
			ip.push(buffer.readUInt32BE((line * recordSize) + (offset * 16) + (i * 4)));
		}
...
module.exports = {
	...
	startWatchingDataUpdate: function (callback) {
		watcher.makeFsWatchFilter(watcherName, geodatadir, 60 * 1000, function () {
			//Reload data
			async.series([
				function (cb) {
					preload(cb);
	...
	stopWatchingDataUpdate: function () {
		watcher.stopWatching(watcherName);
	}
};
